...
test("local HTTP + PostgreSQL: task dates, assignees, edits, isolation and deletion", { skip: !process.env.TASK_TEST_BASE_URL }, async () => {
  ...
  for (const kind of ["case", "application"]) {
    ...
    const task = { id: randomUUID(), title: "Prueba automática prioridades", status: "pending", priority: "Alta", dueDate: "2026-09-30", assigneeId: portfolio.users[1].id };
    ...
    try {
      ...
      assert.equal((await stored()).priority, "Alta");
      assert.equal((await save({ priority: "Urgente" })).status, 400);
      assert.equal((await save({ priority: "Baja" })).status, 200);
      assert.equal((await stored()).priority, "Baja");
