...
export async function conditionalSnapshot(request: Request, scope: string, render: () => Promise<Response>) {
  ...
  const tag = '"' + createHash("sha256").update(JSON.stringify([
    "cost-control-v1", process.env.RAILWAY_GIT_COMMIT_SHA, currentIdentity(), scope,
