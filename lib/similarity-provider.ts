...
export class SimilarityError extends Error { status: number; retryable: boolean; constructor(message: string, status = 502, retryable = false) { super(message); this.status = status; this.retryable = retryable; } }
...
export async function searchSimilar(input: Record<string, unknown>, image?: File, fetcher: typeof fetch = fetch): Promise<SimilarityResult> {
  ...
  if (!response.ok) {
    ...
    throw new SimilarityError(messages[response.status] ?? "La fuente no pudo completar la búsqueda.", [404, 413, 415, 422, 429, 503].includes(response.status) ? response.status : 502, response.status === 429 || response.status >= 500);
  }
