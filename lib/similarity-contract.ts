...
export const proposalSchema = z.object({ name: z.string().trim().max(500).default(""), coverage: coverageSchema.default([]), grouped: z.boolean().default(false), limit: z.number().int().min(1).max(100).default(100), states: z.array(z.enum(['registered','pending','other'])).min(1).max(3).default(['registered','pending']), minSimilarity: z.number().min(0).max(1).default(0) }).strict();
