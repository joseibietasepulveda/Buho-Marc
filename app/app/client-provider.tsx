...
function EditableClientCell({ client, field, label }: { client: Client; field: ClientField; label: string }) {
  ...
  if (!editing) return <button className="buho-client-cell" type="button" aria-label={`Editar ${label} de ${client.name}`} onClick={() => { setDraft(client[field]); setVersion(client.version); setError(""); setEditing(true); }}><span>{client[field] || "Agregar dato"}</span><span className="buho-client-pencil" aria-hidden>✎</span></button>;
