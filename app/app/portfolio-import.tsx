...
const outcomeLabel: Record<string, string> = { ready: "Lista para incorporar", existing: "Ya incorporada", imported: "Incorporada", error: "Revisar", pending: "Pendiente" };
  ...
  async function process(ids: string[], action: "preview" | "import") {
    ...
    try {
      for (let i = 0; i < ids.length && !stop.current; i += 10) {
        ...
        const response = await fetch("/api/portfolio/import", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action, ids: ids.slice(i, i + 10) }) });
