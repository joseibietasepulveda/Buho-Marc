...
export function WatchPanel({ onOpen, onRefresh, onCases, initialQuery = "" }: Props) {
  ...
  return <section className="watch-view">
    ...
    {tab === 'discover' && <p className="watch-help">Los límites reorganizan los resultados al instante. El índice expresa cercanía, no probabilidad de conflicto. Los estados ocultos se controlan mediante la configuración del sistema; se conservan sus antecedentes. INAPI sin publicación significa que la fuente no informa una fecha en el Diario Oficial.</p>}
    ...
    {data?.configured && <>
      ...
      {tab === 'discover' ? groups.map(group => <section key={group.level} className={`watch-band watch-band-${group.level.toLowerCase()}`}><header><h2>{group.level} similitud</h2><span>{group.level === 'Alta' ? `Desde ${settings.high.toFixed(2)}` : `Desde ${settings.medium.toFixed(2)} hasta menos de ${settings.high.toFixed(2)}`}</span></header>{group.rows.length ? group.rows.map(row => <FindingGroup key={`${row.target.id}:${JSON.stringify(settings)} :${query}:${JSON.stringify(publication)}`} {...row} {...groupProps} />) : <p className="watch-empty">No hay hallazgos en este rango con los filtros actuales.</p>}</section>) : <section className="watch-band"><header><h2>Coincidencias en seguimiento</h2><span>Tu selección se conserva al cambiar los límites.</span></header>{followed.length ? followed.map(row => <FindingGroup key={`${row.target.id} :${query}:${JSON.stringify(publication)}`} {...row} {...groupProps} following />) : <p className="watch-empty">Aún no hay coincidencias visibles en seguimiento. Elige una desde Por revisar.</p>}</section>}
    </>}
  </section>;
}
