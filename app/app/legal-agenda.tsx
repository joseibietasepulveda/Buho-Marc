      ...
      <div className="agenda-grid">{Array.from({ length: dayCount }, (_, index) => {
        ...
        return <section key={date} aria-label={displayWorkDate(date)} className={`agenda-day${inPeriod ? "" : " outside-period"}${date === today ? " is-today" : ""}${date === dayFilter ? " is-selected" : ""}`}>
          ...
          {entries.map(event => {
            ...
            return <button className={`agenda-event category-${event.category}${urgent ? " is-urgent" : ""}${urgent && due.tone === "overdue" ? " is-overdue" : ""}`} key={event.id} type="button" title={text} onClick={() => onOpen(event)}><strong>{event.title}</strong><span>{event.context}</span><small>{event.institutional ? "Control de INAPI" : event.informational ? "Hito de referencia" : event.fatal ? "Plazo fatal" : owner(event)}{due.tone === "overdue" ? event.institutional ? " · Revisar demora" : event.informational ? " · Cumplido" : " · Vencido" : event.date === today ? " · Hoy" : ""}</small></button>;
          })}
        </section>;
      })}</div>
    </div>
