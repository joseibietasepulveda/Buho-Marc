...
export function PendingTasks({ cases, users, onCase, compact = false, onAgenda }: {
  cases: { id: string; title: string; brand: string; stage: string; tasks?: CaseTask[] }[];
  compact?: boolean; onAgenda?: () => void; users: TaskMember[]; onCase: (id: string) => void;
}) {
  ...
  if (compact) return <section className="pending-tasks-compact" aria-label="Tareas pendientes de los casos"><div className="pending-compact-header"><strong><Clock size={18} aria-hidden/> {rows.length} tareas pendientes</strong><button type="button" onClick={onAgenda}>Ver agenda →</button></div>{rows.slice(current*3,current*3+3).map(({item,task}) => <button type="button" key={`${item.id}:${task.id}`} className="pending-task-compact" onClick={() => onCase(item.id)}><span><strong>{task.title} · {item.brand}</strong><small>{task.dueDate ? displayWorkDate(task.dueDate) : 'Sin fecha'} · {users.find(u => u.id === task.assigneeId)?.name ?? 'Sin responsable'}</small></span><b className={`task-priority-${taskPriority(task).toLowerCase()}`}>{taskPriority(task)}</b></button>)}{!rows.length && <p>No tienes tareas pendientes.</p>}<div className="pending-compact-footer">{pages > 1 && <><button type="button" disabled={current === 0} onClick={() => setPage(current-1)}>←</button><span>{current+1}/{pages}</span><button type="button" disabled={current === pages-1} onClick={() => setPage(current+1)}>→</button></>}<button type="button" onClick={onAgenda}>Ver todas las tareas ({rows.length}) →</button></div></section>;
