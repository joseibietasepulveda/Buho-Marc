...
export function OppositionDetails({ proceeding, onOpenApplication }: { proceeding: OppositionProceeding; onOpenApplication?: (id: string) => void }) {
  ...
  return <section className="buho-case-section opposition-details">
    <h3>{received ? "Oposición recibida · parte solicitante" : "Oposición presentada · parte oponente"}</h3>
    <dl>
    ...
    <h3>{received ? "Historial de nuestra solicitud" : "Historial del expediente contrario"}</h3>
    {events.length ? <ol>{events.map((e,i) => <li key={`${e.event_id}-${i}`}><strong>{e.event_date?.slice(0,10) ?? "Fecha no informada"} · {e.status_description ?? "Actuación"}</strong>{e.observation && <p>{e.observation}</p>}</li>)}</ol> : <p>La fuente no entregó actuaciones. Revisa el expediente oficial.</p>}
  </section>;
}
