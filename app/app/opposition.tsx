...
export function OppositionDetails({ proceeding }: { proceeding: OppositionProceeding }) {
  ...
  return <section className="buho-case-section opposition-details"><h3>Oposición presentada · parte oponente</h3><dl><dt>Oponente</dt><dd>{proceeding.opponent}</dd><dt>Marca contraria</dt><dd>{record.name} · Solicitud {record.applicationNumber}</dd><dt>Titular de la marca contraria</dt><dd>{record.owner}</dd><dt>Estado del expediente contrario</dt><dd>{statusLabel(record.status)}</dd><dt>Fundamento vinculado</dt><dd>{proceeding.basisName ?? "Sin vínculo"}</dd><dt>Presentación de la oposición</dt><dd>{proceeding.filedAt ?? "Sin fecha registrada"}</dd></dl>{proceeding.note && <p>{proceeding.note}</p>}{proceeding.documentUrl && <a href={proceeding.documentUrl} target="_blank" rel="noreferrer">Abrir respaldo ↗</a>}<p>Las novedades generan una tarea de revisión. Los plazos del oponente requieren verificar la actuación y su notificación; las obligaciones de la parte solicitante no se asignan a este caso.</p><h3>Historial del expediente contrario</h3>{events.length ? <ol>{events.map((e,i) => <li key={`${e.event_id}-${i}`}><strong>{e.event_date?.slice(0,10) ?? "Fecha no informada"} · {e.status_description ?? "Actuación"}</strong>{e.observation && <p>{e.observation}</p>}</li>)}</ol> : <p>La fuente no entregó actuaciones. Revisa el expediente oficial.</p>}</section>;
}
