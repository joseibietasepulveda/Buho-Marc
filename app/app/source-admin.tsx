...
export function SourceAdmin() {
  ...
  const lastCompleted = data?.runs.filter(run => run.status === "success" && run.completed_at).sort((a, b) => b.completed_at!.localeCompare(a.completed_at!))[0];
  const latestRun = data?.runs.slice().sort((a, b) => b.started_at.localeCompare(a.started_at))[0];
  ...
  return <section className="source-admin">
    ...
    {!data && !error && <p role="status">Preparando la fuente y su copia inicial de antecedentes…</p>}
    {tab === "records" && <><div className="source-toolbar"><label>Buscar expediente<input type="search" value={query} onChange={e => { setQuery(e.target.value); setPage(0); }} placeholder="Marca, titular, solicitud o registro" /></label><label>Cartera<select value={filter} onChange={e => { setFilter(e.target.value); setPage(0); }}><option value="all">Todos los expedientes</option><option value="tracked">Solo cartera de la plataforma</option><option value="pending">Con diferencias pendientes</option></select></label>{data && data.provider !== "inapi" && <button className="buho-primary" disabled={busy || !data} onClick={() => void advance()}>{busy ? "Actualizando…" : "Actualizar · generar 6 cambios"}</button>}</div>
      <p className="source-hint">{data?.provider === "inapi" ? "Haz clic en una fila para consultar los datos completos, la cobertura y las actuaciones. Para actualizar la fuente, usa Revisar en Marcas registradas." : "Seleccione un parámetro para editarlo. Los cambios llegan a la plataforma en la siguiente revisión."}</p>
      <div className="source-table-scroll"><table className="source-records"><thead><tr><th>Seguimiento</th>{fields.map(f => <th key={f}>{fieldLabels[f]}</th>)}</tr></thead><tbody>{records.slice(currentPage * 25, (currentPage + 1) * 25).map(row => <tr key={row.id} className={data?.provider === "inapi" ? "source-record-row" : undefined} tabIndex={data?.provider === "inapi" ? 0 : undefined} aria-label={data?.provider === "inapi" ? `Abrir expediente ${row.data.name}` : undefined} onClick={() => { if (data?.provider === "inapi") setInspecting(row); }} onKeyDown={event => { if (data?.provider === "inapi" && event.target === event.currentTarget && ["Enter", " "].includes(event.key)) { event.preventDefault(); setInspecting(row); } }}><td><span className={`source-pill ${row.pending ? "pending" : ""}`}>{row.pending ? "Cambio pendiente" : row.tracked ? "En cartera" : "Fuera de cartera"}</span></td>{fields.map(f => <td key={f}>{data?.provider === "inapi" ? f === "status" ? <span className={`source-pill ${row.data.status === "registered" ? "success" : ""}`}>{statusLabel(row.data.status)}</span> : f === "name" ? <><strong>{row.data.name}</strong><small className="source-record-type">{row.data.type} · Ver ficha →</small></> : f === "classes" ? row.data.classes.join(", ") || "No informadas" : displayValue(row.data[f]) : <button className="source-cell" aria-label={`${data?.provider === "inapi" ? "Ver" : "Editar"} ${fieldLabels[f]} de ${row.data.name}`} onClick={() => data?.provider === "inapi" ? setInspecting(row) : setEditing({ row, field: f })}>{f === "status" ? statusLabel(row.data.status) : f.endsWith("Date") ? sourceDate(row.data[f]) : displayValue(row.data[f])}</button>}</td>)}</tr>)}</tbody></table></div>
      {!records.length && data && <p>No hay expedientes con estos filtros.</p>}
      <footer className="source-pagination"><span>{records.length ? currentPage * 25 + 1 : 0}–{Math.min((currentPage + 1) * 25, records.length)} de {records.length}</span><button disabled={!currentPage} onClick={() => setPage(currentPage - 1)}>Anterior</button><button disabled={(currentPage + 1) * 25 >= records.length} onClick={() => setPage(currentPage + 1)}>Siguiente</button></footer></>}
    {tab === "runs" && <div className="source-runs"><p>Revisiones automáticas a las 12:30 de Chile y revisiones manuales desde Marcas registradas. Se muestran las últimas 100 consultas.</p>{data?.runs.length === 0 && <p className="source-empty">Todavía no hay consultas. La copia inicial de antecedentes ya está preparada; pulse Revisar en la plataforma para comenzar.</p>}{data?.runs.map(run => <details className="source-run" key={run.id}><summary><span><b>{when(run.started_at)}</b><small>{run.trigger === "initial" ? "Carga inicial · sin notificaciones" : run.trigger === "enrollment" ? "Incorporación a cartera" : run.trigger === "manual" ? "Revisión manual" : "Revisión automática"}{run.completed_at ? ` · ${Math.max(0, Math.round((Date.parse(run.completed_at) - Date.parse(run.started_at)) / 1000))} s` : ""}</small></span><span className={`source-pill ${run.status}`}>{runLabel(run.status)}</span><span>{run.received}/{run.requested} expedientes</span><span>{run.changed} actualizados · {run.notifications} avisos</span></summary><div className="source-run-body">{run.error && <p className="source-error">{run.error}. No se aplicaron cambios de esta consulta.</p>}{run.status === "success" && !run.changed && <p>La cartera coincide con la fuente. No se generaron notificaciones nuevas.</p>}{run.detail.map((d, i) => <article key={i}><h3>{d.title}</h3><small>{d.notified ? "Notificación registrada" : "Solo actualización de antecedentes complementarios"}</small><ChangeTable changes={d.changes} /></article>)}<details><summary>Expedientes incluidos en la consulta</summary><p>Solicitudes ({run.request.applicationIds.length}): {run.request.applicationIds.join(", ")}</p><p>Registros ({run.request.registrationIds.length}): {run.request.registrationIds.join(", ") || "Ninguno"}</p></details><small>Referencia de consulta: {run.id}</small></div></details>)}</div>}
    {inspecting && <SourceInspector data={inspecting.data} tracked={inspecting.tracked} pending={inspecting.pending} onClose={() => setInspecting(null)} />}
