...
export async function GET() {
  try {
    ...
    const tasks = await getSql()`SELECT t.id, t.title, t.status, t.priority, t.due_date::text AS due_date, t.assignee_id, a.public_code FROM registration_tasks t JOIN registration_applications a ON a.id = t.application_id WHERE t.organization_id = ${DEMO_ORG_ID} AND a.organization_id = ${DEMO_ORG_ID} ORDER BY t.created_at`;
