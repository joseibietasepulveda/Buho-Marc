...
async function handlePOST(request: Request) {
  ...
  try {
    ...
    await getSql().begin(async tx => {
      ...
      const [member] = await tx`SELECT user_id FROM organization_members WHERE organization_id = ${organizationId()} AND user_id = ${actorId()}`;
      if (!member) throw new EvidenceError("No perteneces a este equipo", 403);
      const [row] = await tx`SELECT a.id, a.data, r.data AS source_data FROM registration_applications a LEFT JOIN source_snapshots s ON s.entity_id = a.id AND s.entity_type = 'application' AND s.organization_id = a.organization_id LEFT JOIN source_records r ON r.id = s.source_id WHERE a.organization_id = ${organizationId()} AND a.public_code = ${input.applicationId} FOR UPDATE OF a`;
      ...
      if (input.action === "revoke") {
        ...
        after.push({ id: randomUUID(), kind: input.kind, method: input.method, date: input.date, reference: input.reference, sourceUrl: input.sourceUrl || undefined, actId: input.actId, actDate: input.actDate, actDescription: input.actDescription, recordedAt: now, recordedBy: actorId() });
      }
      // Only local evidence changes; raw source snapshots and events stay intact.
      await tx`UPDATE registration_applications SET data = ${tx.json({ ...stored, legalEvidence: after })}, updated_at = now() WHERE id = ${row.id} AND organization_id = ${organizationId()}`;
      await tx`INSERT INTO audit_events (organization_id, actor_user_id, action, entity_type, entity_id, before_data, after_data) VALUES (${organizationId()}, ${actorId()}, ${`registration.evidence.${input.action}`}, 'application', ${row.id}, ${tx.json({ legalEvidence: before })}, ${tx.json({ legalEvidence: after })})`;
    });
