...
const schema = z.object({ action: z.enum(["preview", "import"]), ids: z.array(z.string().regex(/^[1-9]\d{0,8}$/)).min(1).max(10) }).strict();
export const POST = withSession(async request => {
  try {
    ...
    for (const id of ids) {
      ...
      results.push({ id, outcome, name: record.name, destination: kind === "brand" ? "Marcas" : "Solicitudes", status: statusLabel(record.status), registrationNumber: record.registrationNumber });
    }
