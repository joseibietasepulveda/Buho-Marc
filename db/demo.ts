...
export async function getDemoSnapshot() {
  ...
  const taskRows = await sql`SELECT t.id, t.title, t.status, t.priority, t.due_at, t.assignee_id, c.public_code FROM case_tasks t JOIN cases c ON c.id = t.case_id WHERE t.organization_id = ${DEMO_ORG_ID} AND c.organization_id = ${DEMO_ORG_ID} ORDER BY t.created_at, t.id`;
