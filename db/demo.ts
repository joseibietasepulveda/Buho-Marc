...
export async function ensureDemoSeed() {
  ...
  if (!seedPromise) seedPromise = seedDemo().catch(error => { seedPromise = undefined; throw error; });
  return seedPromise;
}
async function seedDemo() {
  ...
  await sql.begin(async (tx) => {
    ...
    for (const brand of supplementalSeedBrands.slice(0, Math.max(0, 20 - Number(brandCount.count)))) {
      const inserted = await tx`INSERT INTO brands (id, organization_id, public_code, name, word_mark, owner_name, registration_number, jurisdiction, status, monitoring_config, created_by) VALUES (${brand.id}, ${DEMO_ORG_ID}, ${brand.code}, ${brand.name}, ${brand.name}, ${brand.owner}, ${brand.registration}, 'Chile', ${brand.status}, ${tx.json({ rut: brand.rut, inapiUrl: 'https://buscadormarcas.inapi.cl/Marca/BuscarMarca.aspx', visual: brand.visual })}, ${DEMO_USER_ID}) ON CONFLICT (id) DO NOTHING RETURNING id`;
      if (inserted.length) for (const niceClass of brand.niceClasses) await tx`INSERT INTO brand_classes (brand_id, nice_class) VALUES (${brand.id}, ${niceClass}) ON CONFLICT DO NOTHING`;
    }
    ...
    for (const brand of missingBrands) {
      const inserted = await tx`INSERT INTO brands (id, organization_id, public_code, name, word_mark, owner_name, registration_number, jurisdiction, status, monitoring_config, created_by) VALUES (${brand.id}, ${DEMO_ORG_ID}, ${brand.code}, ${brand.name}, ${brand.name}, ${brand.owner}, ${brand.registration}, 'Chile', 'Activa', ${tx.json({ rut: brand.rut, type: brand.type, visual: brand.type, ...demoBrandDetails(brand.code) })}, ${DEMO_USER_ID}) ON CONFLICT DO NOTHING RETURNING id`;
...
export async function getDemoSnapshot() {
  ...
  const caseRows = await sql`SELECT c.id AS internal_id, c.public_code, c.proceeding, c.title, c.description, COALESCE(b.name, 'Sin marca') AS brand_name, c.client_name, c.stage, c.priority, c.next_deadline, COALESCE(u.name, 'Sin asignar') AS owner_name, m.public_code AS source_match FROM cases c LEFT JOIN brands b ON b.id = c.brand_id LEFT JOIN users u ON u.id = c.owner_id LEFT JOIN matches m ON m.id = c.source_match_id WHERE c.organization_id = ${organizationId()} AND c.status = 'active' AND (${!isRealSource()} OR b.archived_at IS NULL) ORDER BY c.created_at DESC`;
  const userRows = await sql`SELECT u.id, u.name, u.email, u.initials, om.created_at FROM organization_members om JOIN users u ON u.id = om.user_id WHERE om.organization_id = ${organizationId()} ORDER BY om.created_at`;
  const noticeRows = await sql`SELECT m.public_code AS match_code, n.public_code, n.title, n.brand_name, n.urgency, n.managed_at, n.created_at, n.change_detail, d.subject, d.body FROM notifications n LEFT JOIN matches m ON n.entity_type = 'match' AND n.entity_id = m.id AND n.organization_id = m.organization_id JOIN email_drafts d ON d.notification_id = n.id WHERE n.organization_id = ${organizationId()} AND COALESCE(n.change_detail->>'invalidated', 'false') <> 'true' ORDER BY n.created_at DESC`;
  const savedSources = brandRows.some(row => row.monitoring_config?.provider === "inapi")
    ? await sql`SELECT s.entity_id, r.data FROM source_snapshots s JOIN source_records r ON r.id = s.source_id WHERE s.organization_id = ${organizationId()} AND s.entity_type = 'brand' AND r.data->>'provider' = 'inapi'`
    : [];
  const projectedSources = new Map(savedSources.map(row => [row.entity_id, reprojectInapiRecord(row.data as SourceRecord)]));
  const auditRows = await sql`SELECT a.id, a.action, a.entity_type, a.entity_id, a.after_data, a.occurred_at, u.name FROM audit_events a LEFT JOIN users u ON u.id = a.actor_user_id WHERE a.organization_id = ${organizationId()} ORDER BY a.occurred_at DESC LIMIT 200`;

  const visibleFindingRows = await sql`SELECT b.id, count(*)::int AS count FROM matches m JOIN brands b ON b.id = m.brand_id JOIN LATERAL (SELECT result FROM monitoring_jobs j WHERE j.brand_id = b.id AND j.status = 'success' ORDER BY completed_at DESC LIMIT 1) j ON true WHERE b.organization_id = ${organizationId()} AND b.archived_at IS NULL AND m.review_status IN ('Detectada','Pendiente de clasificación') AND j.result->'resultIds' ? m.public_code AND lower(trim(m.evidence->'hit'->>'status')) NOT IN ('denegada','desistida','abandonada') AND (m.evidence->'hit'->>'score')::numeric >= COALESCE((SELECT (watch_settings->>'medium')::numeric FROM organizations WHERE id = b.organization_id), 0.3) GROUP BY b.id`;
  ...
  const [watchSummary] = await sql`SELECT
    ...
    (SELECT count(*)::int FROM matches m JOIN brands b ON b.id = m.brand_id JOIN LATERAL (SELECT result FROM monitoring_jobs j WHERE j.brand_id = b.id AND j.status = 'success' ORDER BY completed_at DESC LIMIT 1) j ON true WHERE m.organization_id = ${organizationId()} AND b.archived_at IS NULL AND m.review_status IN ('Detectada','Pendiente de clasificación') AND j.result->'resultIds' ? m.public_code AND lower(trim(m.evidence->'hit'->>'status')) NOT IN ('denegada','desistida','abandonada') AND (m.evidence->'hit'->>'score')::numeric >= COALESCE((SELECT (watch_settings->>'medium')::numeric FROM organizations WHERE id = b.organization_id), 0.3)) AS detected`;
  return {
    watchSummary,
    audit: auditRows.map(row => ({ id: row.id, date: shortDate(row.occurred_at, true), actor: row.name ?? "Sistema", action: row.action, detail: row.after_data?.applicationNumber ? `Solicitud ${row.after_data.applicationNumber}` : row.entity_type })),
...
export async function resetDemoData() {
  if (!isDemoOrganization()) throw new Error("Solo se pueden restablecer datos de demostración");
  if (isRealSource()) throw new Error("La cartera real no puede restablecerse con datos de demostración");
