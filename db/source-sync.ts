...
export async function syncSource(trigger: "manual" | "scheduled", provider = fetchSource, now = new Date()) {
  ...
  const claim = await sql.begin(async tx => {
    ...
    const targets = await tx`SELECT s.* FROM source_snapshots s LEFT JOIN brands b ON s.entity_type = 'brand' AND b.id = s.entity_id LEFT JOIN registration_applications a ON s.entity_type = 'application' AND a.id = s.entity_id LEFT JOIN cases c ON s.entity_type = 'case' AND c.id = s.entity_id AND c.organization_id = s.organization_id WHERE s.organization_id = ${organizationId()} AND ((s.entity_type = 'brand' AND b.archived_at IS NULL AND (b.status <> 'Pausada' OR b.monitoring_config->>'provider' = 'inapi')) OR (s.entity_type = 'application' AND a.id IS NOT NULL) OR (s.entity_type = 'case' AND c.status = 'active' AND c.stage <> 'Concluido'))`;
