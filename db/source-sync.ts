...
export async function syncSource(trigger: "manual" | "scheduled", provider = fetchSource, now = new Date()) {
  ...
  const claim = await sql.begin(async tx => {
    ...
    await tx`UPDATE source_sync_runs SET status = 'failed', completed_at = now(), error = 'Proceso interrumpido; se liberó la revisión para reintentar' WHERE organization_id = ${organizationId()} AND status = 'running' AND started_at < now() - interval '5 minutes'`;
    const active = await tx`SELECT id FROM source_sync_runs WHERE organization_id = ${organizationId()} AND status = 'running'`;
    if (active.length) return { skip: "Ya hay una revisión en curso" };
    if (trigger === "scheduled") {
      const today = await tx`SELECT status, started_at FROM source_sync_runs WHERE organization_id = ${organizationId()} AND trigger = 'scheduled' AND scheduled_day = ${clock.day} ORDER BY started_at DESC`;
    ...
    const targets = await tx`SELECT s.* FROM source_snapshots s LEFT JOIN brands b ON s.entity_type = 'brand' AND b.id = s.entity_id LEFT JOIN registration_applications a ON s.entity_type = 'application' AND a.id = s.entity_id LEFT JOIN cases c ON s.entity_type = 'case' AND c.id = s.entity_id AND c.organization_id = s.organization_id WHERE s.organization_id = ${organizationId()} AND ((s.entity_type = 'brand' AND b.archived_at IS NULL AND (b.status <> 'Pausada' OR b.monitoring_config->>'provider' = 'inapi')) OR (s.entity_type = 'application' AND a.id IS NOT NULL AND COALESCE(a.data->>'portfolioRole', 'own') <> 'third-party') OR (s.entity_type = 'case' AND c.status = 'active' AND c.stage <> 'Concluido'))`;
    ...
    const [run] = await tx`INSERT INTO source_sync_runs (organization_id, trigger, scheduled_day, status, requested, request) VALUES (${organizationId()}, ${trigger}, ${trigger === "scheduled" ? clock.day : null}, 'running', ${input.applicationIds.length}, ${tx.json(input)}) RETURNING id`;
    ...
    return await sql.begin(async tx => {
      ...
      for (const target of targets!) {
        ...
        if (target.entity_type === "brand") {
          const [brand] = await tx`SELECT monitoring_config FROM brands WHERE id = ${target.entity_id} AND organization_id = ${organizationId()} FOR UPDATE`;
